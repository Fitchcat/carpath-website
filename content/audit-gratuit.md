# Contenu — Page `/audit-gratuit`

> Produit par : Copywriter  
> Source : SITEMAP.md · WIREFRAMES.md · USER_FLOW.md · context/BRAND_CONTEXT.md  
> Statut : texte final, prêt à être designé.

---

## SEO

**Meta title** (56 car.) :  
`Réserver mon audit gratuit | CARPATH — 30 minutes, sans engagement`

**Meta description** (138 car.) :  
`Réservez votre audit gratuit avec CARPATH. 30 minutes, un premier bilan à l'oral, puis l'audit détaillé envoyé par écrit. Sans engagement.`

---

## [HEADER SIMPLIFIÉ]

- Logo CARPATH (lien vers la home)
- Lien discret : ← Retour au site

*(Pas de menu complet sur cette page — focus total sur la réservation.)*

---

## [HERO CONVERSION]

**H1 :**  
Réservez votre audit gratuit

**Sous-titre :**  
En 30 minutes, nous faisons le point sur votre situation : vos freins de croissance, vos priorités, ce qu'un système de croissance pourrait changer pour vous. Vous repartez avec un premier bilan à l'oral, puis l'audit détaillé complet vous est envoyé par écrit dans les jours qui suivent — qu'on travaille ensemble ensuite ou non.

---

## [ÉLÉMENTS DE RÉASSURANCE]

**Élément 1 — Gratuit et sans engagement**  
Titre : Gratuit, sans condition  
Texte : L'audit ne vous engage à rien. C'est un échange, pas un processus de vente. Si notre façon de travailler ne correspond pas à votre situation, on se le dit.

**Élément 2 — Durée maîtrisée**  
Titre : Environ 30 minutes  
Texte : Pas plus. Nous respectons votre temps de dirigeant. La session est cadrée, structurée, efficace.

**Élément 3 — Ce que vous repartez avec**  
Titre : Un premier bilan, puis l'audit complet par écrit  
Texte : À la fin de l'échange, vous repartez avec un premier bilan résumé, à l'oral. Dans les jours qui suivent, vous recevez l'audit détaillé complet par écrit — vos principaux freins de croissance et les piliers à activer en priorité. C'est déjà utile, même si vous ne donnez pas suite.

---

## [EMBED TIDYCAL]

*[Bloc technique — intégration widget TidyCal. L'UI Designer et le Front-End Engineer gèrent l'embed. Texte d'accompagnement ci-dessous.]*

**Titre au-dessus du widget :**  
Choisissez un créneau qui vous convient

**Sous-titre :**  
Les créneaux disponibles sont mis à jour en temps réel. Une confirmation vous est envoyée par email immédiatement après la réservation.

---

## [POST-CONVERSION]

*(Ce message s'affiche uniquement après confirmation de la réservation par TidyCal.)*

**Titre :**  
C'est réservé. Voici la suite.

**Contenu :**  
Vous recevez un email de confirmation dans les prochaines minutes. Il contient le lien de l'appel vidéo (ou les modalités de l'échange, selon le format choisi).

Pour préparer notre échange, il peut être utile d'avoir en tête :

- Votre chiffre d'affaires actuel et son évolution sur les 12 derniers mois
- Vos principales sources de nouveaux clients aujourd'hui
- Ce qui freine votre développement selon vous

Pas d'obligation — c'est simplement ce qui nous permet d'aller plus vite à l'essentiel.

À la fin de l'échange, vous repartez avec un premier bilan à l'oral. L'audit détaillé complet vous est envoyé par écrit dans les jours qui suivent.

**Mention :**  
À très bientôt.  
L'équipe CARPATH

---

## FAQ

**Q : Est-ce vraiment gratuit ?**  
Oui, sans condition. L'audit ne génère aucun frais, aucun abonnement automatique, aucune relance commerciale agressive. C'est un échange, pas un entonnoir de vente dissimulé.

**Q : Que se passe-t-il après l'audit ?**  
Si l'adéquation est là des deux côtés, nous vous présentons une proposition de partenariat adaptée à votre situation. Si ce n'est pas le bon moment ou le bon contexte, nous le disons clairement.

**Q : À qui s'adresse cet audit ?**  
Aux dirigeants de TPE et PME en activité, entre 150 000 € et 2 M€ de CA, qui veulent développer leur entreprise avec méthode. Si vous n'êtes pas dans ce profil, la page [Pour qui ?](/pour-qui) vous aidera à en avoir le cœur net.

**Q : L'appel se passe comment ?**  
En visioconférence, depuis votre bureau. Nous partageons parfois un document ou un schéma en cours d'échange. Aucun logiciel à installer au préalable.

